...
void PhysicsSystem::update(float deltaTime){
    ...
    for (auto eid : compMgr.getEntitiesWith<PhysicsComponent>()) {
      auto phys = compMgr.getComponent<PhysicsComponent>(eid);
      auto pos  = compMgr.getComponent<PositionComponent>(eid);
      // 물리 스텝, 충돌 검사, 이벤트 발행 등…
      eventMgr.pushCollision(...);
    }
